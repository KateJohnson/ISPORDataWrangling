...
1. Become familiar with a project oriented workflow and R markdown document.
2. Perform simple data manipulations using the 5 dplyr verbs (`select`, `filter`, `arrange`, `mutate`, `summarise`, `group_by`). Chain these operations together using piping (`%>%`).
...
(Aside: Check out this [this](https://www.tidyverse.org/articles/2017/12/workflow-vs-script/) blogpost by Jenny Bryan for best practices for getting your data into R. Hint: if you use code like this: 'rm(list=ls())' she will "set your computer on fire." 
...
## Topic 2: Data manipulations using dplyr
...
`select()` has a lot of handy tricks:
  ...
  - Repoistion variables based on their order in `select()`
...
Okay, we've got `select` and `filter()` covered. Let's slip in `arrange()` quickly.

```{r}
arrange(diabetes, ID)
```

We can arrange by multiple variables, and if you want it the other way, just `desc()`!

I want to order by BMI, and then from oldest to youngest.
...
What's happening here? Let's write it here:
...
Let's stop here and give you a chance to play with your new tools. 
